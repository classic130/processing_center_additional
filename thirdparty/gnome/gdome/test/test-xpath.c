...
test_xpath (GdomeDocument *doc) {
  ...
  if (strcmp (temp_str->str, "p"))
    fprintf(stderr,"\n1st xpath result tagName, %s != p",temp_str->str);
  ...
  temp_str = gdome_el_tagName((GdomeElement *)gnode, &exc);
  if (strcmp (temp_str->str, "p"))
    fprintf(stderr,"\n2nd xpath result tagName, %s != p",temp_str->str);
  gdome_n_unref(gnode, &exc);
	gdome_str_unref(temp_str);
  dot();
  ...
  gnode = gdome_xpresult_iterateNext(res, &exc);
  if (gnode != NULL)
    fprintf(stderr,"\n3nd xpath result received, none expected");
  gdome_n_unref(gnode, &exc);
  dot();
  ...
  res = gdome_xpeval_evaluate(eval, str, (GdomeNode *)doc, nsresolv, 0, NULL, &exc);
	...
	if (result_type != GDOME_ORDERED_NODE_ITERATOR_TYPE)
		fprintf(stderr,"\nResult type not node set, got %d",result_type);
	dot();

  gnode = gdome_xpresult_iterateNext(res, &exc);
  temp_str = gdome_el_tagName((GdomeElement *)gnode, &exc);
  if (strcmp (temp_str->str, "foo:a1"))
    fprintf(stderr,"\n1st xpath result tagName, %s != foo:a1",temp_str->str);
  gdome_n_unref(gnode, &exc);
  ...
  if (strcmp (temp_str->str, "foo:a2"))
    fprintf(stderr,"\n2nd xpath result tagName, %s != foo:a2",temp_str->str);
  gdome_n_unref(gnode, &exc);
	gdome_str_unref(temp_str);
	gdome_xpresult_unref(res, &exc);
  dot();
  ...
  res = gdome_xpeval_evaluate(eval, str, (GdomeNode *)doc, NULL, 0, NULL, &exc);
	...
	if (result_type != GDOME_NUMBER_TYPE)
		fprintf(stderr,"\nResult type not number, got %d",result_type);
	dot();

	number_value = gdome_xpresult_numberValue(res, &exc);
	if (number_value != 2)
		fprintf(stderr,"\nResult set count != 1, got %f",number_value);
	dot();
  ...
  res = gdome_xpeval_evaluate(eval, str, (GdomeNode *)doc, NULL, 0, NULL, &exc);
	...
	if (result_type != GDOME_BOOLEAN_TYPE)
		fprintf(stderr,"\nResult type not boolean type, got %d",result_type);
	dot();

	boolean_value = gdome_xpresult_booleanValue(res, &exc);
	if(boolean_value != 1)
		fprintf(stderr,"\nBoolean value != 1, = %d",boolean_value);
	dot();
  ...
  gnode = gdome_xpresult_iterateNext(res, &exc);
	...
	if (result_type != GDOME_XPATH_NAMESPACE_NODE)
		fprintf(stderr, "\nresult_type != namespace node, = %d", result_type);

	/* prefix */
	temp_str = gdome_xpns_prefix((GdomeXPathNamespace *)gnode, &exc);
  if (strcmp (temp_str->str, "foo"))
    fprintf(stderr,"\nnamespace prefix, %s != foo",temp_str->str);
	gdome_str_unref(temp_str);
  ...
  if (strcmp (temp_str->str, "http://foo.com/baz"))
    fprintf(stderr,"\nnamespace URI, %s != http://foo.com/baz",temp_str->str);
	gdome_str_unref(temp_str);
	gdome_str_unref(str);
	gdome_n_unref(gnode, &exc);
	gdome_xpresult_unref(res, &exc);
