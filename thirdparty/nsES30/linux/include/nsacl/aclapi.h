...
struct ACLDispatchVector {
     ...
     *
     * A UserDB is corresponding to the USERDB tag in server.xml
     * It translates an ACL's database name to a dbswitch.conf database name
     * for one particular VS.
     * In order to get, for instance, the database handle from an ACL, you now
     * have to get hold of the VS that handles this request first. It can be found
     * in the "resource" plist under ACL_ATTR_VS.
     * Using the VS and the ACL's database name, ACL_UserDBLookup will find a UserDB
     * object, from which you can get the DbHandle, DBType and DbName.
     *
     * Unfortunately, the virtualization made a few functions obsolete because
     * there is no immediate correspondence between the ACL's authinfo->database
     * attribute and the list of databases from dbswitch.conf anymore.
     *
     * Here's a list:
     * ACL_AuthInfoGetDbType() - does not have VS
     * ACL_AuthInfoGetDbName() - does not have VS
     * ACL_DatabaseFind() - will still find dbswitch.conf databases by name
     *
     * The UserDB object can also store name-value pairs (Attrs). Most notably, a value
     * for baseDN can be stored there. One needs to hold the read lock while
     * accessing the Attrs, and the write lock while updating them.
     */
    int (*f_ACL_UserDBLookup)(NSErr_t *errp, VirtualServer *vs, const char *name, UserDB **userdb);
    int (*f_ACL_UserDBGetAttr)(NSErr_t *errp, UserDB *userdb, const char *name, char const **value);
    int (*f_ACL_UserDBSetAttr)(NSErr_t *errp, UserDB *userdb, const char *name, const char *value);
    void (*f_ACL_UserDBReadLock)(UserDB *userdb);
    void (*f_ACL_UserDBWriteLock)(UserDB *userdb);
    void (*f_ACL_UserDBUnlock)(UserDB *userdb);
    void (*f_ACL_UserDBGetDbHandle)(NSErr_t *errp, UserDB *userdb, void **db);
    void (*f_ACL_UserDBGetDbType)(NSErr_t *errp, UserDB *userdb, ACLDbType_t *dbtype);
    void (*f_ACL_UserDBGetDbName)(NSErr_t *errp, UserDB *userdb, const char **name);
};
...
#define ACL_UserDBLookup (*__nsacl_table->f_ACL_UserDBLookup)
#define ACL_UserDBGetAttr (*__nsacl_table->f_ACL_UserDBGetAttr)
#define ACL_UserDBSetAttr (*__nsacl_table->f_ACL_UserDBSetAttr)
#define ACL_UserDBReadLock (*__nsacl_table->f_ACL_UserDBReadLock)
#define ACL_UserDBWriteLock (*__nsacl_table->f_ACL_UserDBWriteLock)
#define ACL_UserDBUnlock (*__nsacl_table->f_ACL_UserDBUnlock)
#define ACL_UserDBGetDbHandle (*__nsacl_table->f_ACL_UserDBGetDbHandle)
#define ACL_UserDBGetDbType (*__nsacl_table->f_ACL_UserDBGetDbType)
#define ACL_UserDBGetDbName (*__nsacl_table->f_ACL_UserDBGetDbName)
