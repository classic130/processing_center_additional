...
DECLARE_INTERFACE_(IPersistFolder3, IPersistFolder2)
{
    ...
    STDMETHOD(InitializeEx)(THIS_ IBindCtx *pbc, DWORD dwRes, LPCITEMIDLIST pidl) PURE;
};
...
#define CSIDL_WINDOWS                   0x0024        // GetWindowsDirectory()or SYSROOT
#define CSIDL_SYSTEM                    0x0025        // GetSystemDirectory()
#define CSIDL_PROGRAM_FILES             0x0026        // C:\Program Files
#define CSIDL_MYPICTURES                0x0027        // My Pictures
#define CSIDL_PROFILE                   0x0028        // USERPROFILE
#define CSIDL_SYSTEMX86                 0x0029        // Get the x86 system directory on RISC
#define CSIDL_PROGRAM_FILESX86          0x002a        // x86 Program Files directory on RISC
...
#if (_WIN32_IE >= 0x0400)
SHSTDAPI_(BOOL)    SHGetSpecialFolderPathA(HWND hwnd, LPSTR lpszPath, int csidl, BOOL fCreate);
SHSTDAPI_(BOOL)    SHGetSpecialFolderPathW(HWND hwnd, LPWSTR lpszPath, int csidl, BOOL fCreate);
...
SHSTDAPI SHGetFolderPathA(HWND hwnd, int csidl, HANDLE hToken, DWORD dwReserved, LPSTR lpszPath);
SHSTDAPI SHGetFolderPathW(HWND hwnd, int csidl, HANDLE hToken, DWORD dwReserved, LPWSTR lpszPath);
...
SHSTDAPI SHGetFolderLocation(HWND hwnd, int csidl, HANDLE hToken, DWORD dwReserved, LPITEMIDLIST *ppidl);
...
// IShellFolder BindCtx parameters. the IUnknown for these are
...
typedef struct tagEXTRASEARCH
{
    ...
    WCHAR   wszUrl[2084];
    WCHAR   wszIcon[MAX_PATH+10]; // "name.dll,#" where # is icon index in the file name.dll
...
// [member functions]
//
// IShellFolder2::GetUIObjectOf2(hwndOwner, cidl, apidl, riid, prgfInOut, ppvOut, uFlags)
//   Same as IShellFolder2::GetUIObjectOf except in case of riid = IID_IContextMenu
// when, depending on the flags, Context Menu is created with items for File or
// Edit menu only or both
//
...
//
// IShellFolder2::EnumSearches(LPENUMEXTRASEARCH *ppenum)
//   gives an enumerator of the searches to be added to the search menu
//--------------------------------------------------------------------------

// IShellFolder2::GetUIObjectOf2 flags
#define SFGUIOO2_DEFAULT   0   // both items for file and edit menu included
#define SFGUIOO2_EDIT      1   // gets context menu for merge with edit menu only


// IShellFolder2::GetItemData() nFormat values
#define SFGID_FINDDATA        1 // WCHAR version
#define SFGID_NETRESOURCE     2 // WCHAR version
#define SFGID_DESCRIPTIONID   3 // no strings
...
DECLARE_INTERFACE_(IShellFolder2, IShellFolder)
{
    ...
    STDMETHOD(GetUIObjectOf2)   (THIS_ HWND hwndOwner, UINT cidl, LPCITEMIDLIST * apidl,
                                 REFIID riid, DWORD dwType, UINT * prgfInOut, void **ppvOut) PURE;
    STDMETHOD(GetDefaultSearchGUID)(THIS_ LPGUID lpGuid) PURE;
    STDMETHOD(EnumSearches)(THIS_ LPENUMEXTRASEARCH *ppenum) PURE;

    // params just like SHGetDataFromIDList(), TCHAR versions converted for you
    STDMETHOD(GetItemData)(THIS_ LPCITEMIDLIST pidl, int nFormat, void *pv, int cb) PURE;
};
...
DECLARE_INTERFACE_(IDockingWindowFrame, IOleWindow)
{
    ...
    STDMETHOD(FindToolbar) (THIS_ LPCWSTR pwszItem, REFIID riid, void **ppvObj) PURE;
};
...
typedef struct {
    ...
    BOOL bDefault;      // OUT is column on by default?
    WCHAR wszTitle[MAX_COLUMN_NAME_LEN]; // OUT the title of the column
} SHCOLUMNINFO;

typedef struct {
    LPCITEMIDLIST pidl; // IN  the absolute pidl (caller still owns!)
    VARIANT vData;      // OUT the data for the pidl
} SHCOLUMNDATA;

#undef INTERFACE
#define INTERFACE IColumnProvider

// Note: People who implement this interface must be threadsafe!  GetItemData _will_ be called
...
DECLARE_INTERFACE_( IColumnProvider, IUnknown )
{
    ...
    STDMETHOD_(ULONG, GetColumnCount)(THIS) PURE;
    STDMETHOD (GetColumnInfo)(THIS_ DWORD dwIndex, SHCOLUMNINFO *psci) PURE;
    STDMETHOD (GetItemData)(THIS_ const SHCOLUMNID *pscid, SHCOLUMNDATA *pscd) PURE;
};
...
#define SHCNEE_ORDERCHANGED         2L  // dwItem2 is the pidl of the changed folder
#define SHCNEE_MSI_CHANGE           4L  // dwItem2 is the product code
#define SHCNEE_MSI_UNINSTALL        5L  // dwItem2 is the product code
#endif
...
SHSTDAPI SHGetDataFromIDListA(LPSHELLFOLDER psf, LPCITEMIDLIST pidl, int nFormat, void *pv, int cb);
SHSTDAPI SHGetDataFromIDListW(LPSHELLFOLDER psf, LPCITEMIDLIST pidl, int nFormat, void *pv, int cb);
