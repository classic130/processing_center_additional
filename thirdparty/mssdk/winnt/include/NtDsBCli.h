...
// Type of Backup passed to HrBackupPrepare()
...
// Type of Restore passed to HrRestorePrepare()
...
#define HrIsNTDSOnline                      HrIsNTDSOnlineW
...
#define HrSetAuthIdentity                   HrSetAuthIdentityW

#else

#define HrIsNTDSOnline                      HrIsNTDSOnlineA
...
#define HrSetAuthIdentity                   HrSetAuthIdentityA
...
Routine Description:
  ...
  Arguments:
    ...
    [out]   ppvExpiryToken - pointer that will receive the pointer to the
                ...
                BackupFree() API by the caller when it is no longer needed.
...
Routine Description:
  ...
  Arguments:
    ...
    [out]   pszAttachmentInfo - pointer that will receive the pointer to the attachment
                info; allocated memory should be freed using BackupFree() API by the
...
Routine Description:
  ...
  Arguments:
    ...
    [out]   pszBackupLogFiles - pointer that will receive the pointer to the list of
                log files; allocated memory should be freed using BackupFree() API by the
...
Routine Description:
  ...
  Arguments:
    [in]    hbc - backup context handle which would have been obtained through
                    HrBackupPrepare() in the backup case and through HrRestorePrepare()
                    in the restore case.
    [out]   pszDatabaseLocationList - pointer that will receive the pointer to the list of
                database locations; allocated memory should be freed using BackupFree() API by the
...
Routine Description:
      ...
      HrRestoreRegister
        ...
        to HrRestoreRegisterComplete() is made.
