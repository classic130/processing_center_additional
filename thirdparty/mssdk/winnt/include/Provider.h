//
// Provider.h -- declaration of Provider class
//
...
{
	// This class needs to access some protected/private methods
	// we don't want to publish to just anyone.

	friend class CWbemProviderGlue;

	public:
		Provider( const CHString& setName, LPCTSTR pszNameSpace = NULL );
		~Provider();
	...
	protected:
		...
		// execute a method
		virtual HRESULT ExecMethod(const CInstance& pInstance, const BSTR bstrMethodName, CInstance *pInParams, CInstance *pOutParams, long lFlags = 0L);

		// find and create all instances of your class
		virtual HRESULT EnumerateInstances(MethodContext*  pMethodContext, long lFlags = 0L) =0;

		// called by the framework to allow derived classes to fill in 
		// properties not filled by the base class
		// override only if you are deriving from an existing provider
		virtual void GetExtendedProperties(CInstance* pInstance, long lFlags = 0L)
		{
		}
	
		// you will given an object with the key properties filled in
		// you need to fill in all of the rest of the properties
		virtual HRESULT GetObject(CInstance* pInstance, long lFlags = 0L) = 0;
		...
		// UNLESS you pass it off to commit
		...
		// do not delete the pointer once committed.
		...
		// you can override the following to support flags above 
		// and beyond those listed in FlagDefs below
	...
	private:
		...
		BOOL ValidateWBEMObjectPointer( MethodContext*  pMethodContext );	// This function ensures that our IWBEMObject*
												// pointer is available, and gets called
												// by the framework entrypoint functions

		/* Interfaces For Use by the Framework         */
		/* You Will Probably Never Call These Directly */

		HRESULT GetObject(	ParsedObjectPath* pParsedObjectPath, 
							MethodContext* pContext, long lFlags = 0L );

		HRESULT ExecuteQuery( MethodContext* pContext, CFrameworkQuery &pQuery, long lFlags = 0L);
		...
		HRESULT ExecQueryAsync(BSTR QueryLanguage,
							 BSTR Query,
							 long lFlags,
							 MethodContext* pContext )
								{ return E_NOTIMPL; };
		...
		static void InitComputerName( void );

		// Sets an instance key from a parsed object path.
		BOOL SetKeyFromParsedObjectPath( CInstance* pInstance, ParsedObjectPath* pParsedObjectPath );
