...
#if (_WIN32_IE >= 0x0500)
#define RBS_EX_OFFICE9      0x00000001  // new gripper, chevron, focus handling
#endif
...
#if (_WIN32_IE >= 0x0500)
#define RB_SETBANDFOCUS (WM_USER + 40)  // (UINT) wParam == band index      lParam == TRUE/FALSE
                                        // returns TRUE if gave band focus, else FALSE
#define RB_GETBANDFOCUS (WM_USER + 41)  // returns index of band with focus (-1 if none)
#define RB_CYCLEFOCUS   (WM_USER + 42)  // (UINT) wParam == band index      (BOOL) lParam == back/forward
                                        // returns index of band that got focus (-1 if none)
#define RB_SETEXTENDEDSTYLE (WM_USER + 43)
#endif      // _WIN32_IE >= 0x0500
...
#if (_WIN32_IE >= 0x0500)
#define RBN_SETFOCUS        (RBN_FIRST - 9)     // Uses NMREBAR
...
#define TreeView_SetItemState(hwndTV, hti, data, mask) \
{ TVITEM _ms_TVi;\
