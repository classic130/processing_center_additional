...
int CMP_InitExponentTable 
  (int, CMPInt *, CMPWord, CMPInt *, int **, CMPInt **, CMP_SURRENDER_CTX *);
...
CMPInt *modulus,
CMPInt *result,
CMP_SURRENDER_CTX *surrenderContext)
{
  CMPInt *expTab = (CMPInt *)NULL_PTR;
  CMPInt tmp;
  CMPWord n0Prime, power;
  register int i, unroll;
  int status, bitIndex;
  int *expExists = (int *)NULL_PTR;
  ...
  do {
    /* Check the surrender function before beginning. */
    ...
    if ((status = CMP_InitExponentTable
         (unroll, modulus, n0Prime, base, &expExists, &expTab,
          surrenderContext)) != 0)
      break;
    ...
    /* Compute exponent by successively squaring partial product "unroll"
      ...
      */
    while (bitIndex >= 0) {
      /* Check the surrender function before each iteration. */
      if ((status = CheckSurrender (surrenderContext)) != 0)
        break;
      ...
      for (i = 1; i <= min (unroll, bitIndex + 1); ++i) {
        ...
        /* result = p[k] ^ (2i) */
        if ((status = VS_CMP_Move (&tmp, result)) != 0)
          break;
      }
...
/* Returns a right adjusted bitfield of length n equal to bits pos, pos - 1,
     pos - 2, ..., pos - n + 1
 */
CMPWord CMP_GetBitsFromWord (
CMPWord srcWord, /* word from which bit field is to be extracted */
int pos,         /* starting position for extracted bits         */
int n)           /* length of extracted bit field                */
{
...
/* Returns a right adjusted bitfield of length = min (n, *pos + 1) equal
     ...
     of a CMPWord.
 */
CMPWord CMP_GetBitsFromCMPInt(
CMPInt *x,       /* CMPInt from which bit field is to be extracted */
int *pos,        /* starting position for extracted bits           */
int length)      /* length of extracted bit field                  */
{
 ...
 */
CMPWord CMP_GetMSBitFromWord (CMPWord srcWord)
{
  CMPWord msb;
 ...
 */
int CMP_ComputeLogOfTableSize (int lengthInBits)
{
  if (lengthInBits >= 64)
    return (4);
  if (lengthInBits >= 16)
...
CMP_SURRENDER_CTX *surrenderContext)  /* the surrender context             */
{
  int i, status;
  int tableSize;
  CMPInt unit;
  ...
  /* The incoming table addresses must be NULL_PTRs.
   */
  if ( (*expExists != (int *)NULL_PTR) || (*expTab != (CMPInt *)NULL_PTR) )
    return (CMP_INVALID_ARGUMENT);

  tableSize = 1 << logSize;
  CMP_Constructor (&unit);
  do {
    *expExists = (int *)T_malloc (tableSize * sizeof (int));
    if (*expExists == (int *)NULL_PTR) {
      status = CMP_MEMORY;
      break;
    }
     ...
     */
    *expTab = (CMPInt *) T_malloc (tableSize * sizeof (CMPInt));
    if (*expTab == (CMPInt *) NULL_PTR) {
      status = CMP_MEMORY;
      break;
    }
    ...
    /* Check the surrender function before computing the next entries. */
    if ((status = CheckSurrender (surrenderContext)) != 0)
      break;
 ...
 */
void CMP_DestructExponentTable(int logSize, int **expExists, CMPInt **expTab)
{
  int i, tableSize;

  tableSize = 1 << logSize;

  /* If the incoming expExists table is NULL_PTR, do nothing, if not,
       zeroize the memory and free it up.
   */
  if (*expExists != (int *)NULL_PTR) {
    T_memset ((POINTER)*expExists, 0, tableSize * sizeof (int));
    T_free ((POINTER)(*expExists));
    *expExists = (int *)NULL_PTR;
  }

  /* If the incoming exponent table is NULL_PTR, do nothing, if not,
       destruct all the entries.
   */
  if (*expTab != (CMPInt *)NULL_PTR) {
    for (i = 0; i < tableSize; i++)
      CMP_Destructor (*expTab + i);
    T_memset ((POINTER)(*expTab), 0, tableSize * sizeof (CMPInt));
    T_free ((POINTER)(*expTab));
    *expTab = (CMPInt *)NULL_PTR;
  }
}
...
/* VS_CMP_ModExpCRT - decrypt ciphertext c into message m using Chinese
     Remainder Theorem.
...
CMP_SURRENDER_CTX *surrenderContext)
{
  ...
  do {
    ...
    if ((status = VS_CMP_ModExp
         (result, exponentP, primeP, &temp, surrenderContext)) != 0)
      break;
    ...
    if ((status  = CMP_ModularReduce (base, primeQ, result)) != 0)
      break;
    if ((status = VS_CMP_ModExp
         (result, exponentQ, primeQ, &resultQ, surrenderContext)) != 0)
      break;
    ...
    /* Add resultQ */
    status = VS_CMP_AddInPlace (&resultQ, result);
