...
CMPInt *montRep)
{
  ...
  do {
     ...
     */
    if ((status = VS_CMP_Move (sourceCMPInt, &srcXMod)) != 0)
      break;

    if ((status = VS_CMP_ShiftLeftByCMPWords (sizeShift + 1, &srcXMod)) != 0)
      break;
 ...
 */
int CMP_ConvertFromMont (CMPInt *montRep,
CMPInt *modulus,
CMPWord n0Prime,
CMPInt *reducedCMP)
{
 ...
 */
int CMP_MontSquare(CMPInt *operand,CMPInt *modulus,CMPWord n0Prime,CMPInt *montSquare)
{
  register int i;
  ...
  T_memset ((POINTER)sqval, 0, (mLength2 + 3) * CMP_BYTES_PER_WORD);
  ...
  if ((status = VS_CMP_RecomputeLength (mLength2, montSquare)) != 0)
    return (status);

  /* Find montSquare * 2, do this by shifting one bit to the left. */
  if ((status = VS_CMP_ShiftLeftByBits (1, montSquare)) != 0)
    return (status);
  ...
  VS_CMP_ShiftRightByCMPWords (mLength, montSquare);
  VS_CMP_RecomputeLength (mLength, montSquare); 

  /* Subtract modulus from montSquare if possible.
   */
  sqToMCompare = VS_CMP_Compare (modulus, montSquare);
  if (sqToMCompare < 0){
    if ((status = VS_CMP_SubtractInPlace (modulus, montSquare)) != 0)
      return (status);
  }
  else {
    if (sqToMCompare == 0) {
...
CMPInt *montProduct)
{
  ...
  T_memset ((POINTER)pval, 0, (mLength2 + 3) * CMP_BYTES_PER_WORD);
  ...
  /* Extract upper half of result */
  VS_CMP_ShiftRightByCMPWords (mLength, montProduct);
  VS_CMP_RecomputeLength (mLength, montProduct); 

  /* Subtract modulus from montProduct if possible
   */
  pToMCompare = VS_CMP_Compare (montProduct, modulus);
  if (pToMCompare >= 0){
    if ((status = VS_CMP_SubtractInPlace (modulus, montProduct)) != 0)
      return (status);
  }
