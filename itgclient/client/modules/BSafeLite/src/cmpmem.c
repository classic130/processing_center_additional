...
/* This routine constructs, or initializes a CMPInt to be 0 space allocated,
     0 length and a NULL_PTR for the value.
 */
void CMP_Constructor (CMPInt *theInt)
{
 ...
 */
void CMP_Destructor (CMPInt *theInt)
{
  if (CMP_PINT_VALUE (theInt) != CMP_NULL_VALUE) {
    T_memset ((POINTER)CMP_PINT_VALUE (theInt), 0,
              (unsigned int)(CMP_PINT_SPACE (theInt) * sizeof (CMPWord)));
    T_free ((POINTER)CMP_PINT_VALUE (theInt));
    CMP_PINT_VALUE (theInt) = CMP_NULL_VALUE;
  }
  CMP_PINT_SPACE (theInt) = 0;
  CMP_PINT_LENGTH (theInt) = 0;
...
int CMP_realloc (int wordsNeeded,CMPInt *theInt)
{
  ...
  /* If value is a null pointer, this is a new CMPInt. If T_malloc is
  ...
  if (CMP_PINT_VALUE (theInt) == CMP_NULL_VALUE) {
    ...
    if ((CMP_PINT_VALUE (theInt) = (CMPWord *)T_malloc (bytesNeeded))
         != (CMPWord *)NULL_PTR)
      return (0);
    CMP_PINT_SPACE (theInt) = 0;
    return (CMP_MEMORY);
   ...
   */
  if ((newValue = (CMPWord *)T_malloc (bytesNeeded)) != (CMPWord *)NULL_PTR) {
    T_memcpy ((POINTER)newValue, (POINTER)CMP_PINT_VALUE (theInt),
