...
/* Copies one CMPInt to another.
     destination <--- source
 */
int VS_CMP_Move (CMPInt *source, CMPInt *destination)
{
  ...
  /* Copy with T_memcpy. */
  T_memcpy
    ((POINTER)(CMP_PINT_VALUE (destination)),
     (POINTER)(CMP_PINT_VALUE (source)),
...
/* Compares two CMPInt's. The return value is
     ...
     VS_CMP_Compare (a, b) > 0
      means           a > b
 */
int VS_CMP_Compare (CMPInt *firstInt,CMPInt *secondInt)
{
  ...
  /* If the lengths are different, the one with the longer length is
       greater. If they are the same, compare word by word. T_memcmp
...
/* Adds two CMPInt's.
     sum = addend1 + addend2
*/
int VS_CMP_Add (CMPInt *addend1,CMPInt *addend2,CMPInt *sum)
{
...
/* Increments a CMPInt by the amount given in a CMPWord.
     base = base + increment
*/
int VS_CMP_AddCMPWord (CMPWord increment,CMPInt *base)
{
 ...
 */
int VS_CMP_Subtract (CMPInt *minuend,CMPInt *subtrahend,CMPInt *difference)
{
 ...
 */
int VS_CMP_SubtractCMPWord (CMPWord decrement,CMPInt *base)
{
