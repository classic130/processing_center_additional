...
/* Adds two CMPInt's, placing the result into the second argument.
     base = base + increment
*/
int VS_CMP_AddInPlace (CMPInt *increment,CMPInt *base)
{
  ...
  if (CMP_PINT_LENGTH (base) < CMP_PINT_LENGTH (increment)) {
    ...
    T_memset ((POINTER)&(CMP_PINT_WORD (base, CMP_PINT_LENGTH (base))), 0,
      (unsigned int)((wordCount - CMP_PINT_LENGTH (base)) * sizeof (CMPWord)));
...
/* Subtracts two CMPInt's, placing the result into the second argument.
     base = base - decrement
*/
int VS_CMP_SubtractInPlace (CMPInt *decrement,CMPInt *base)
{
 ...
 */
int VS_CMP_ShiftLeftByCMPWords (int shiftCount,CMPInt *base)
{
  CMPInt oldBase;
  int status;
  ...
  if (CMP_PINT_SPACE (base) < totalLen) {
    do {
      ...
      if ((status = VS_CMP_Move (base, &oldBase)) != 0)
        break;
      ...
      /* Set the first shiftCount words (the least significant words)
           to be 0. */
      T_memset
        ((POINTER)(CMP_PINT_VALUE (base)), 0, shiftCount * CMP_BYTES_PER_WORD);

      /* Copy the words from oldBase into base, beginning after the zeros. */
      T_memcpy ((POINTER)&(CMP_PINT_WORD (base, shiftCount)),
                (POINTER)(CMP_PINT_VALUE (&oldBase)),
  ...
  T_memmove ((POINTER)&(CMP_PINT_WORD (base, shiftCount)),
             (POINTER)(CMP_PINT_VALUE (base)), bLen * CMP_BYTES_PER_WORD);

  /* Set the first shiftCount words (the least significant words) to be 0. */
  T_memset ((POINTER)(CMP_PINT_VALUE (base)), 0,
...
*/
int VS_CMP_ShiftLeftByBits (int leftShift,CMPInt *base)
{
  ...
  if (leftShift >= CMP_WORD_SIZE) {
    if ((status = VS_CMP_ShiftLeftByCMPWords
 ...
 */
int VS_CMP_ShiftRightByCMPWords (int shiftCount,CMPInt *base)
{
#ifdef CMP_DEBUG
  if (CMP_PINT_VALUE (base) == CMP_NULL_VALUE)
    return (CMP_INVALID_ADDRESS);
  ...
  if (CMP_PINT_LENGTH (base) > 0) {
    ...
    T_memmove ((POINTER)(CMP_PINT_VALUE (base)),
               (POINTER)&(CMP_PINT_WORD (base, shiftCount)),
...
/* Shifts base rightShift bits to the right. Any bits shifted off the end
     are lost forever.
 */
int VS_CMP_ShiftRightByBits (int rightShift,CMPInt *base)
{
  int status;
 ...
 */
int VS_CMP_RecomputeLength (int targIndex,CMPInt *base)
{
  register CMPWord *bval;
 ...
 */
int VS_CMP_GetOffsetOfMSB (CMPInt *theInt,int *offset)
{
