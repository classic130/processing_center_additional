 ...
 */
int CMP_ModAdd (CMPInt *addend1,CMPInt *addend2,CMPInt *modulus,CMPInt *sum)
{
  int status;
  CMPInt reducedSum;
  ...
  do {
    if ((status = VS_CMP_Add (addend1, addend2, sum)) != 0)
      break;

    /* If the result of the addition is < modulus, done.
     */
    if (VS_CMP_Compare (sum, modulus) < 0)
      break;

    /* Otherwise, reduce.
     */
    if ((status = VS_CMP_SubtractInPlace (modulus, sum)) != 0)
      break;
    ...
    if ((status = CMP_ModularReduce (sum, modulus, &reducedSum)) != 0)
      break;
    status = VS_CMP_Move (&reducedSum, sum);
  } while (0);
...
CMPInt *difference)
{
  ...
  do {
    if (VS_CMP_Compare (minuend, subtrahend) >= 0) {
      ...
      status = VS_CMP_Move (&reducedDifference, difference);
      break;
    }
     ...
     */
    if (VS_CMP_Compare (&reducedDifference, modulus) <= 0) {
      status = VS_CMP_Subtract (modulus, &reducedDifference, difference);
    }
     ...
     */
      ...
      if ((status = VS_CMP_Move (&reducedDifference, difference)) != 0)
        break;
      if ((status = CMP_ModularReduce
           (difference, modulus, &reducedDifference)) != 0)
        break;
