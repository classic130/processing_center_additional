 ...
 */
int CMP_BitLengthOfCMPInt (CMPInt *theInt)
{
...
/* Gets the bitPosition-th bit of theInt, setting the return variable
     ...
     set or not.
 */
int CMP_GetBit (int bitPosition,CMPInt *theInt,int *theBit)
{
 ...
 */
int CMP_ClearBit (int bitPosition,CMPInt *theInt)
{
  int wordIndex, bitIndex;
  ...
  if (CMP_PINT_LENGTH (theInt) > wordIndex) {
    ...
    VS_CMP_RecomputeLength (CMP_PINT_LENGTH (theInt) - 1, theInt);
    return(0);
  }
 ...
 */
int CMP_SetBit (int bitPosition,CMPInt *theInt)
{
  int wordIndex, bitIndex, status;
  ...
  T_memset ((POINTER)&CMP_PINT_WORD (theInt, CMP_PINT_LENGTH (theInt)), 0,
    (CMP_PINT_SPACE (theInt) - CMP_PINT_LENGTH (theInt)) * CMP_BYTES_PER_WORD);

  /* Set the bit. */
  CMP_PINT_WORD (theInt, wordIndex) = ((CMPWord)1 << bitIndex);
  VS_CMP_RecomputeLength (wordIndex, theInt);
