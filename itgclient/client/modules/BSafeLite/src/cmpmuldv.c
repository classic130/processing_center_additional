...
int CMP_UpdateCoefficients 
  (CMPInt *, CMPInt *, CMPInt *, int *, int *, CMPInt *, CMPInt *);

/* Local support routines for division and modular reduction.
 */
int CMP_AppendWord 
  (CMPWord, CMPInt *);
int CMP_EstimateMSWQuotient 
  (CMPInt *, CMPInt *, CMPInt *, CMPInt *);
void CMP_DivideTwoWordsByWord 
  (CMPWord, CMPWord, CMPWord, CMPWord*, CMPWord *);
void CMP_DivOneAndHalfWordsByWord 
  (CMPWord, CMPWord, CMPWord, CMPWord *, CMPWord *);
 ...
 */
int CMP_Multiply (CMPInt *multiplicand,CMPInt *multiplier,CMPInt *product)
{
  ...
  T_memset ((POINTER)pval, 0, prodLength * CMP_BYTES_PER_WORD);
...
CMPInt *remainder)
{
  ...
  do { 
     ...
     */
    if (VS_CMP_Compare (dividend, divisor) < 0) {
      status = VS_CMP_Move (dividend, remainder);
      break;
    }
     ...
     */
    if ((status = VS_CMP_GetOffsetOfMSB (divisor, &shift)) != 0)
      break;
    ...
    if (shift > 0) {
      if ((status = VS_CMP_ShiftLeftByBits (shift, divisor)) != 0)
        break;
      if ((status = VS_CMP_ShiftLeftByBits (shift, dividend)) != 0)
        break;
    }
     ...
     */
    if ((status = VS_CMP_Move (dividend, remainder)) != 0)
      break;
    if (aLength > bLength) {
       if ((status = VS_CMP_ShiftRightByCMPWords
     ...
     */
      ...
      CMP_PINT_LENGTH (quotient) = 1;
      if ((status = VS_CMP_SubtractInPlace (divisor, remainder)) != 0)
        break;
    }
    ...
    for (j = aLength - bLength - 1; j >= 0; --j) {
       ...
       */
      if (VS_CMP_Compare (remainder, divisor) >= 0) {
         if ((status = VS_CMP_SubtractInPlace (divisor, remainder)) != 0)
           break;
         if ((status = VS_CMP_AddCMPWord ((CMPWord)1, quotient)) != 0)
           break;
      }
      ...
      if ((status = CMP_AppendWord
        ...
        break;
      if ((status = VS_CMP_ShiftLeftByCMPWords (1, quotient)) != 0)
        break;
      ...
      /* Adjust qj and the product so that it is less than the new partial
           divisor.
       */
      while (VS_CMP_Compare (&product, remainder) > 0) {
        if ((status = VS_CMP_SubtractCMPWord ((CMPWord)1, &qj)) != 0)
          break;
        if ((status = VS_CMP_SubtractInPlace (divisor, &product)) != 0)
          break;
      }
      if (status != 0)
        break;

      if ((status = VS_CMP_AddCMPWord (CMP_INT_WORD (qj, 0), quotient)) != 0)
        break;
      if ((status = VS_CMP_SubtractInPlace (&product, remainder)) != 0)
        break;
    }
    if (status != 0)
      break;
     ...
     */
    if (shift > 0) {
      VS_CMP_ShiftRightByBits (shift, remainder);
      VS_CMP_ShiftRightByBits (shift, dividend); 
      VS_CMP_ShiftRightByBits (shift, divisor);
    }
...
CMPInt *reducedValue)
{
  ...
  do { 
     ...
     */
    if (VS_CMP_Compare (operand, modulus) < 0) {
      status = VS_CMP_Move (operand, reducedValue);
      break;
    }
    ...
    /* Compute left shift that will make the most significant bit of the
         ...
         operand accordingly
     */
    if ((status = VS_CMP_GetOffsetOfMSB (modulus, &shift)) != 0)
      break;
    ...
    if (shift > 0) {
      if ((status = VS_CMP_ShiftLeftByBits (shift, modulus)) != 0)
        break;
      if ((status = VS_CMP_ShiftLeftByBits (shift, operand)) != 0)
        break;
    }
     ...
     */
    if ((status = VS_CMP_Move (operand, reducedValue)) != 0)
      break;
    if (aLength > bLength) {
       if ((status = VS_CMP_ShiftRightByCMPWords
     ...
     */
      ...
      if ((status = VS_CMP_SubtractInPlace (modulus, reducedValue)) != 0)
        break;
    }
    ...
    for (j = aLength - bLength - 1; j >= 0; --j) {
      /* If the current reducedValue is > modulus, subtract modulus from
           the reducedValue.
       */
      if (VS_CMP_Compare (reducedValue, modulus) >= 0) {
         if ((status = VS_CMP_SubtractInPlace (modulus, reducedValue)) != 0)
           break;
      }
       ...
       */
      while (VS_CMP_Compare (&product, reducedValue) > 0) {
        if ((status = VS_CMP_SubtractInPlace (modulus, &product)) != 0)
          break;
      }
      if (status != 0)
        break;

      if ((status = VS_CMP_SubtractInPlace (&product, reducedValue)) != 0)
        break;
    }
    if (status != 0)
      break;
     ...
     */
    if (shift > 0) {
      VS_CMP_ShiftRightByBits (shift, reducedValue);
      VS_CMP_ShiftRightByBits (shift, operand); 
      VS_CMP_ShiftRightByBits (shift, modulus);
    }
...
/* Bump each CMPWord of a up 1, then set the least significant word to x.
     ...
     ---- ---- ---- ---- ----
 */
int CMP_AppendWord (CMPWord x,CMPInt *a)
{
  int status, aLength;
  ...
  if (CMP_PINT_WORD (a, aLength - 1) != (CMPWord)0) {
    ...
    T_memmove ((POINTER)&(CMP_PINT_WORD (a, 1)),
               (POINTER)CMP_PINT_VALUE (a), aLength * CMP_BYTES_PER_WORD);
    CMP_PINT_WORD (a, 0)  = x;
    CMP_PINT_LENGTH (a)++;
    return (0);
  }
...
CMPInt *product)  
{
  ...
  do {
     ...
     */
    if ((status = VS_CMP_Move (numerator, &partNum)) != 0)
      break;
    if ((status = VS_CMP_Move (divisor, &partDiv)) != 0)
      break;

    VS_CMP_ShiftRightByCMPWords (max (numeratorLength - 3, 0), &partNum);
    VS_CMP_ShiftRightByCMPWords (max (divisorLength - 2, 0), &partDiv);
...
int CMP_ComputeGCD (CMPInt *u,CMPInt *v,CMPInt *gcd)
{
  int status, gcdFlag;
  CMPInt temp1, temp2;
  ...
  do {
    /* Initialize temp1 to be the larger of the two operands, and gcd to be
         the smaller.
     */
    if (VS_CMP_Compare (u, v) >= 0) {
      if ((status = VS_CMP_Move (u, &temp1)) != 0)
        break;
      if ((status = VS_CMP_Move (v, gcd)) != 0)
        break;
    }
    else {
      if ((status = VS_CMP_Move (v, &temp1)) != 0)
        break;
      if ((status = VS_CMP_Move (u, gcd)) != 0)
        break;
    }
    ...
    switch (gcdFlag) {
      case 1:
        status = VS_CMP_Move (&temp1, gcd);
	break;

      case 2:
        status = VS_CMP_Move (&temp2, gcd);
	break;
...
CMPInt *gcd)
{
  ...
  do {
    ...
    if (VS_CMP_Compare (u, v) >= 0) {
      biggerCMP  = u;
      smallerCMP = v;
      if ((status = VS_CMP_Move (u, &dividendP)) != 0)
        break;
      if ((status = VS_CMP_Move (v, &divisorP)) != 0)
        break;
      ...
      biggerCMP  = v;
      smallerCMP = u;      
      if ((status = VS_CMP_Move (v, &dividendP)) != 0)
        break;
      if ((status = VS_CMP_Move (u, &divisorP)) != 0)
        break;       
	...
	break;
       ...
       */
      if ((status = VS_CMP_Move (biggerCMP, gcd)) != 0)
        break;
    ...
    if (CMP_PINT_WORD (gcd, CMP_PINT_LENGTH (gcd) - 1) == (CMPWord)0) {
      ...
      CMP_PINT_LENGTH (coeffB) = 1;
      CMP_PINT_WORD (coeffB, 0) = (CMPWord)1;
      status = VS_CMP_Move (smallerCMP, gcd);
      break;
    }
    ...
    switch (divideFlag) {
      case 0:
        status = VS_CMP_Move (&divisorP, gcd);
        break;

      case 2:
        status = VS_CMP_Move (&dividendP, gcd);
    ...
    if (signCoeffA > 0) {
      if ((status = VS_CMP_Subtract (&temp1, gcd, &temp2)) != 0)
        break;
    }
    else {
      if ((status = VS_CMP_Add (&temp1, gcd, &temp2)) != 0)
        break;
    }
     ...
     */
    if (signCoeffA > 0) {
      if ((status = VS_CMP_Subtract (biggerCMP, coeffB, &temp2)) != 0)
        break;
      if ((status = VS_CMP_Move (&temp2, coeffB)) != 0)
        break;
    }
    /* if coeffB > 0 then coeffA < 0 so then coeffA <--- -coeffA mod smallerCMP
     */
    else {
      if ((status = VS_CMP_Subtract (smallerCMP, coeffA, &temp2)) != 0)
        break;
      if ((status = VS_CMP_Move (&temp2, coeffA)) != 0)
        break;
    }
  ...
  if ( (uComparedToV == U_SMALLER_THAN_V) && (status == 0) ) {
    do {
      if ((status = VS_CMP_Move (coeffA, &temp1)) != 0)
        break;
      if ((status = VS_CMP_Move (coeffB, coeffA)) != 0)
        break;
      if ((status = VS_CMP_Move (&temp1, coeffB)) != 0)      
        break;
    } while (0);
  }
...
CMPInt *an1)
{
  ...
  *sign_n = *sign_n1;
  ...
  do {
     ...
     */
    if (*sign_n1 != anSign) {
      if ((status = VS_CMP_Add (an, an1Xq, an2)) != 0)
        break;
      *sign_n1 = -(*sign_n1);
    }
    ...
    /* an <-- an1,   an1 <-- an2
     */
    if ((status = VS_CMP_Move (an1, an)) != 0)
      break;
    if ((status = VS_CMP_Move (an2, an1)) != 0)
      break;  
  } while (0);
 ...
 */
int CMP_AbsDifference (
CMPInt *a,
CMPInt *b,
int *signDiff,
CMPInt *difference)
{
  if (VS_CMP_Compare (a, b) >= 0) {
    *signDiff = 1;
    return (VS_CMP_Subtract (a, b, difference));
  }
  else {
    *signDiff = -1;
    return (VS_CMP_Subtract (b, a, difference));
  }
}
