...
/* CMP_OctetStringToCMPInt () copies an octet string vector into a CMPInt,
  ...
  */
int CMP_OctetStringToCMPInt (unsigned char *octetString,
unsigned int octetStringLen,
CMPInt *theInt)
{
  int status, theIntLen, fullWords;
  register int i, j;
  register CMPWord *wordPtr;
  register unsigned char *bytePtr;

#ifdef CMP_DEBUG
  if (octetString == (unsigned char *)NULL_PTR)
 ...
 */
int CMP_CMPWordToCMPInt(CMPWord sourceWord,CMPInt *destInt)
{
  int status;
 ...
 */
int CMP_PowerOfTwo (int exponent,CMPInt *theInt)
{
  int status;
  ...
  T_memset
    ((POINTER)(CMP_PINT_VALUE (theInt)), 0, wordIndex * CMP_BYTES_PER_WORD);
