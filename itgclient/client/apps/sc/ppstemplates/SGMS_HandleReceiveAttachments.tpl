<!--  TW_AccDeliveryPage.tpl line 1664  -->
...
{
	// adding the header
	var tab = '<table border="0" valign="TOP" width="100%" cellspacing="1" cellpadding="2" bgcolor="#cccccc">\
				...
				<td nowrap align="center" width="*" >\
					<table border="0" cellpadding="0" cellspacing="0">\
						<tr>\
							...
							<a href="javascript:SaveAttachment(\''+files[i]+'\')" class="buttonText" tabindex="1">\
...
function saveCheckedAttachments()
{
	...
	for (var i=0;i<document.form1.elements.length;i++) {
		...
		if (e.name == 'file_checkbox' && e.checked) {
...
function getFileSize( filename )
{
	...
	if( size < 1024 ){
		// Bytes
		suffix = "Bytes";// < % =tw.locals.BYTES;%>";
	} else if( size < 1024*1024 ){
		// KB
		suffix = "KB"; // "< % =tw.locals.KB;%>";
		size /= 1024;
	} else if( size < (1024*1024*1024) ){
		// MB
		suffix = "MB"; // "< % =tw.locals.MB;%>";
		size /= (1024*1024);
	} else {
		// GB
		suffix = "GB"; // "< % =tw.locals.GB;%>";
		alert( size );
		size /= 1024*1024*1024;
	}
	if (size < 10)
