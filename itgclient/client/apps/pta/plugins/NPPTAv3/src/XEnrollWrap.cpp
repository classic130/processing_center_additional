...
EXTERN_C DWORD Enroll_get_ProviderType(ICEnroll* pEnroll)
{
	...
	hr = pEnroll->get_ProviderType((LONG*)&retVal);
	_lastErrCode = hr;
...
EXTERN_C DWORD Enroll_get_GenKeyFlags(ICEnroll* pEnroll)
{
	...
	hr = pEnroll->get_GenKeyFlags((LONG*)&retVal);
	_lastErrCode = hr;
...
EXTERN_C DWORD Enroll_get_KeySpec(ICEnroll* pEnroll)
{
	...
	hr = pEnroll->get_KeySpec((LONG*)&dwKeySpec);
	_lastErrCode = hr;
...
{
	HRESULT hr;
	LONG longDeleteCertRequest;
	BOOL boolDeleteCertRequest;

	assert(pEnroll != NULL);

	hr = pEnroll->get_DeleteRequestCert(&longDeleteCertRequest);
	_lastErrCode = hr;


	if(hr!=S_OK)
		longDeleteCertRequest = FALSE;


	return (DWORD)longDeleteCertRequest;
}
...
EXTERN_C DWORD Enroll_get_WriteCertToCSP(ICEnroll* pEnroll)
{
	...
	LONG longWriteCertToCSP;

	assert(pEnroll != NULL);

	hr = pEnroll->get_WriteCertToCSP(&longWriteCertToCSP);
	_lastErrCode = hr;

	if(hr!=S_OK)
		longWriteCertToCSP = FALSE;

	return (DWORD)longWriteCertToCSP;
}
...
EXTERN_C wchar_t* Enroll_createPKCS10(ICEnroll* pEnroll, wchar_t* pwstrDN, int nDNLen, wchar_t* pwstrUsage, int nUsageLen)
{
	...
	catch(...)
	{
		...
		else
		{
			...
			{
				DWORD flags;  
				pEnroll->get_GenKeyFlags((LONG*)&flags);
				if(flags & 0x04000000)
