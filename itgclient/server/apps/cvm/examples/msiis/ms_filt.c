...
/*
		      vs_cvm_genlib_log_message(LOG_FAILURE, "Problems acquiring CRLs. Some not installed.");
*/
...
/*
	         ...
	         vs_cvm_genlib_log_message(LOG_FAILURE, buf);
		  } else {
			if (globalLogInfo == 1)
				vs_cvm_genlib_log_message(LOG_INFO, "Successfully refreshed certificates and CRLs.");
		  }
	  }
    }
    else {
      vs_cvm_genlib_log_message(LOG_FAILURE, "Could not acquire mutex");
    }
*/
    /* And release the mutex. */
/*
    if (ReleaseMutex(hMutex)==0) {
      vs_cvm_genlib_log_message(LOG_FAILURE, "Error releasing mutex");
    }
*/
...
int vsCheckCertInit() {
  ...
  do {
	...
	//	vs_cvm_genlib_log_message(LOG_FAILURE, "%s", errorString);
	//	status = 0;
    ...
    //vs_cvm_genlib_log_message(LOG_INFO, "%s starting", FILTER_DESCRIPTION);
...
int vsCheckClientCert(HTTP_FILTER_CONTEXT *pFC, int *reqFinished,
  ...
  //  }
  //	vs_cvm_genlib_log_message(LOG_FAILURE, refreshStatusString);
  //    }
  //  }      
  ...
  **/
  ...
  if (status) {
    ...
    //vs_cvm_genlib_log_message(LOG_FAILURE, "%s", refreshStatusString);

    /* Log denials? */
    //if (globalLogSecurity && certStatus) {
    //  vs_cvm_genlib_log_message(LOG_SECURITY, "Certificate %s for %s", 
    ...
    *reqFinished = 1;
    ...
    //    vs_cvm_genlib_log_message(LOG_ACCESS, "auth-user %s", userName ? userName : "client");
    //  }
    //}
    //else {
    if (certStatus != CS_VALID) {
...
**/
  ...
  //if (globalLogDebug == 1)
  //  vs_cvm_genlib_log_message(LOG_DEBUG, "TerminateFilter::");
...
DWORD HandleEvent_Authentication(HTTP_FILTER_CONTEXT *pFC, PHTTP_FILTER_AUTHENT pAuthData) {
  ...
  if ( ( pFC->pFilterContext==(void *) VS_RESOURCE_NOT_PROTECTED) ||
	   ...
	   ( pFC->pFilterContext==(void *) VS_RESOURCE_PROTECTED_BUT_AUTH_FAIL) ) {
	  if (globalLogDebug == 1)
		vs_cvm_genlib_log_message(VS_LOG_DEBUG, "AUTH-EVENT: Resource is not protected or already authenticated");
	  return dwRet;
  }
...
DWORD HandleEvent_AuthComplete(HTTP_FILTER_CONTEXT *pFC, PHTTP_FILTER_AUTH_COMPLETE_INFO pAuthInfo)
{
  ...
  if (WaitForSingleObject(hMutex, INFINITE)==WAIT_OBJECT_0)
  {
    ...
    if (newCertLen == 0)
    {
      vs_cvm_genlib_log_message (VS_LOG_FAILURE, "AUTH-EVENT: Could not get certificate - ");
      vs_cvm_genlib_log_message (VS_LOG_FAILURE, "Please check that you enabled SSL Listner and Checked Client Certificate Authentication in ISA Incoming Web Request Tab");
      vs_cvm_genlib_log_message (VS_LOG_FAILURE, "You can also get this error if you do not have ISA SP1 and hotfix for Q319375 installed.");
      return vsSendErrorPage(pFC, 103, 0);
    }
