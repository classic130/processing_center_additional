  ...
  /* and register its cleanup function */
	...
	*/
	...
	/*break;*/
     ...
     logFileName = pblock_findval("log-file-name",pb);
     if (logFileName == NULL){
	log_error(LOG_WARN, "vsCheckCertInit", sn, rq, "log-file-name parameter not set in magnus.conf ");
	return REQ_ABORTED;
      }
     if ((cp.logError=fopen(logFileName,"a")) == 0) {
	log_error(LOG_FAILURE, "vsCheckCertInit", sn, rq, "Failed to open log file %s [system error message: %s]", logFileName, strerror(errno));
	return REQ_ABORTED;
      }
...
/*  Fix for CR29977 - Begin */
      ...
      if (pblock_findval("check-chain", pb)){ 
          cp.noCheckChain = pblock_findval("check-chain", pb);
          ...
          if ( pblock_findval("no-check-chain", pb)) {
             cp.noCheckChain = pblock_findval("no-check-chain", pb); 
             log_error(LOG_WARN, VS_FN_NAME_INIT, sn, rq, NO_CHECK_CHAIN_USAGE_WARNING);
         }
