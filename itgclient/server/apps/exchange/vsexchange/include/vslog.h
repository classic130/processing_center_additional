/*
 * $Revision: 1.1.6.1 $
 * $Header: /cvs/itgclient/server/apps/exchange/vsexchange/include/Attic/vslog.h,v 1.1.6.1 2003/06/06 22:46:31 npaul Exp $
 */
...
#define VSFUNCRETURNLOG(msg,status) \
    if (status != 0) VSAA_Log (VS_LOG_ERROR, __LINE__, __FILE__, \
        (char *) "%s returns error 0x%x", msg, status)
...
VS_STATUS VSAA_Log (VS_LOG_TYPE level, int line, const char *filename,
           char* format, ...);

VS_STATUS VSAA_LogW(VS_LOG_TYPE level, int line, const char *filename,
           wchar_t* format, ...);
/* VS_DataLog 
...
#define VSLOGERROR(msg) \
    VSAA_Log (VS_LOG_ERROR, __LINE__, __FILE__, msg)

#define VSLOG(level, msg) \
    VSAA_Log (level, __LINE__, __FILE__, msg)

#define VSTRACE(msg) \
    VSAA_Log (VS_LOG_DEBUG, __LINE__, __FILE__, (char *) ">>> %s", msg)
