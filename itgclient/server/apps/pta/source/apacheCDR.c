/*
$Revision: 1.3.2.3 $
$Header: /cvs/itgclient/server/apps/pta/source/apacheCDR.c,v 1.3.2.3 2003/08/17 03:02:43 sgoginen Exp $
*/
...
/*
 * Similar to ap_table_set, but may set multiple env vars (or none) based upon
 * value of env_options parameter
 */
void vs_table_CDR_env_set(table *tbl, const char *name, const char *value, apacheCDR_EnvOptions env_options)
{
  int i;
  ...
  for (i=0; apacheCDR_EnvVarTable[i].ssl_name != 0 ; i++) {
    if ( strcmp(apacheCDR_EnvVarTable[i].ssl_name, name) == 0 ) {
      ...
      if ( env_options.use_vs_name == 1) {
        ap_table_set(tbl, cdr_env->vs_name, value);       
      }
      if ( env_options.use_compat_name == 1) {
        ap_table_set(tbl, cdr_env->compat_name, value);
      }
      return;
    }
  }
...
/*
 * Similar to ap_table_set, but sets {varname}nn values
 * Replaces {nn} found in the name with stringified value of the nn arg.
 * note: only works for nn in the range 0...99
 */
void vs_table_nn_set(table *tbl, const char *name, int nn, const char *value)
{
  int i;
  char *tmp_name;
  char *snn;
  ...
  ap_table_set(tbl, tmp_name, value);
  free(tmp_name);
}

/*
 * Similar to ap_table_set, but sets OUnn values for multiple env vars (or none)
 ...
 */
void vs_table_CDR_env_nn_set(table *tbl, const char *name, const char *value,
...
int VSUnwrapCertificate(request_rec *req)
{
  ...
  certB64 = (char *) ap_table_get(req->subprocess_env, "SSL_CLIENT_CERT"); /* nsapi: rq->vars auth-cert */
  if (!certB64) {
#ifdef DEBUG
