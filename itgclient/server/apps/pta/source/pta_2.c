...
int myFileRead(apr_pool_t *pool, char * filename, char ** data, int * datalen) {
  FILE *fp = NULL;
  int status = 0, cnt = 0, bytes = 0,bytes1 = 0;
  ...
  do {
      ...
      do {
        bytes = fread(buf, 1, 4096, fp);
        cnt+=bytes;
      } while (bytes>0);
      *datalen = cnt;
      if ((*data = (char *)apr_pcalloc(pool, (*datalen) + 1))==NULL) {
         status = 10000;
      break;
    }
  rewind(fp);
  ...
  if (status) { *datalen = 0; *data = 0; }
  fflush(stderr);
  if (fp) fclose(fp);
  return status;
}

/* Function that renders the protected page 
*  This has been added for Apache2.0.52    */
int vsPTARenderPage(char **outBuf, apr_pool_t *pool, char *filename) {
  int status = 0;
char * data = NULL;
int datalen = 0;
  do {
if(status = myFileRead(pool, filename, &data, &datalen)) {
break;
}
if (datalen > 0)
*outBuf = data;
