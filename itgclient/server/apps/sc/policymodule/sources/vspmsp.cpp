/*
 * $Revision: 1.1.2.1 $
 * $Header: /cvs/itgclient/server/apps/sc/policymodule/sources/Attic/vspmsp.cpp,v 1.1.2.1 2001/09/06 22:32:35 ajayaram Exp $
*/
...
////////////////////////////////////////////////////////////////////
{
	...
	char *ecasInstallDir = NULL;
	size_t defaultTemplateLen = 0;

	do {

		ecasInstallDir = getenv(ECASINSTALL);
		if ((NULL == ecasInstallDir) || (strlen(ecasInstallDir) == 0)) {
			defaultTemplateLen = strlen(FILE_PROTOCOL) +
								 strlen(PROTOCOL_DELIMITER) +
								 strlen(DEFAULT_ECAS_INSTALL) +
								 strlen(DEFAULT_POLICY_TEMPLATE) + 1;
			defaultTemplate = (char *) malloc(defaultTemplateLen);
			if (NULL == defaultTemplate) {
				*status = VSPM_OUT_OF_MEMORY;
				break;
			}

			memset(defaultTemplate, 0, defaultTemplateLen);
...
////////////////////////////////////////////////////////////////////
{
	...
	do {
		...
		if (! verFlagItem) {
			...
			verFlagStr = (const char *) verFlagItem->value();
			if (! verFlagStr) {
...
////////////////////////////////////////////////////////////////////
{
	...
	do {
		...
		callBackFunc = (void (*)(VSPM_Container, VSPM_STATUS *)) GetProcAddress (libHandle,
                                        funcName);
		if (!callBackFunc) {
			LPVOID lpMsgBuf;
			FormatMessage( 
			...
			fprintf(stderr, "GetProcAddress failed: %s %s\n", funcName, (LPCTSTR)lpMsgBuf);
			LocalFree( lpMsgBuf );
...
////////////////////////////////////////////////////////////////////
{
	...
	do {
		...
		*status = VSPM_STATUS_NO_ERROR;
		...
		nvpItem = new VSNvp(nvpCnt, name, nameLen, (const unsigned char*) value, valueLen);
		if (! nvpCnt) {
			*status = VSPM_OUT_OF_MEMORY;
			break;
		}
...
////////////////////////////////////////////////////////////////////
{
	...
	do {
		...
		*status = VSPM_STATUS_NO_ERROR;
		...
		if ((! oldName) || (strcmp((const char *) oldName, (const char *) cntName) != 0)) {
			*status = VSPM_OUT_OF_MEMORY;
			break;
		}
...
////////////////////////////////////////////////////////////////////
{
	...
	do {
		...
		*status = VSPM_STATUS_NO_ERROR;
		...
		item->setValue((const unsigned char*) value, valueLen);
		oldValue = NULL;
		oldValue = (char *) item->value();
